...
class NginxProxyManagerAPI {
  ...
  async verifyToken() {
    try {
      ...
      const response = await this.client.get('/api/tokens/verify', { timeout: 5000 });
      return response.data;
    } catch (error) {
      // Fallback: try a simple authenticated request to check if token works
      try {
        await this.client.get('/api/tokens', { timeout: 5000 });
        return { valid: true };
      } catch (fallbackError) {
